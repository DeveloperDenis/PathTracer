...
void init_demo_scene(World* world, Camera* camera, f32 aspectRatio)
{
    assert(world);
    assert(camera);
...
// TODO: there is a bug in this scene with random cyan pixels near the bottom of the sphere?
void init_test_scene_1(World* world, Camera* camera, f32 aspectRatio)
{
    assert(world);
    assert(camera);
    
    Material testMaterial = Material::diffuse(v4f(1.0f, 1.0f, 1.0f, 1.0f));
    
    world->add_sphere(v3f(0.0f, 0.0f, -4.0f), 1.0f, &testMaterial);
    
    v3f cameraPos = v3f();
    *camera = Camera(cameraPos, 40.0f, aspectRatio);
}
